    ...
    if(partners){
        return (
            <React.Fragment>
                ...
                <Media body className='ml-5 mb-4'><Media heading >{partners.name}</Media>{partners.description}</Media>
            </React.Fragment>
        );
    }
  }
...
function About(props) {
    ...
    const partners = props.partners.map(partner => {
        return (
            <Media tag='li' key={partner.id} ><RenderPartner partners={partner} /></Media>
        );
    });
